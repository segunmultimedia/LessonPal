...
// ... (keep the same generateContent function)
function generateContent(description: string, topic: string) {
  ...
  if (d.includes('fraction')) {
  ...
  } else if (d.includes('multiply') || d.includes('multiplication')) {
  ...
  } else if (d.includes('divide') || d.includes('division')) {
  ...
  } else if (d.includes('add') || d.includes('addition') || d.includes('sum')) {
  ...
  } else if (d.includes('subtract') || d.includes('subtraction') || d.includes('difference')) {
  ...
  } else if (d.includes('shape') || d.includes('geometry') || d.includes('angle')) {
  ...
  } else if (d.includes('time') || d.includes('clock') || d.includes('minute')) {
  ...
  } else if (d.includes('measure') || d.includes('length') || d.includes('mass') || d.includes('weight') || d.includes('capacity') || d.includes('volume') || d.includes('perimeter') || d.includes('area')) {
  ...
  } else if (d.includes('data') || d.includes('graph') || d.includes('chart') || d.includes('tally')) {
  ...
  } else if (d.includes('algebra') || d.includes('pattern') || d.includes('sequence') || d.includes('equation') || d.includes('unknown')) {
    ...
    // Generic high-quality mathematics template for unspecified topics (mostly basic numbers operations / counting)
  ...
  const cleanDesc = d.split('core competencies')[0].split('e.g.')[0].replace(/\n/g, ' ').replace(//g, '').trim();
  const objText = cleanDesc.length > 150 ? cleanDesc.substring(0, 147) + '...' : cleanDesc;
  const learningObjective = `By the end of the lesson, pupils will be able to ${objText}`;
...
async function generateAll() {
  ...
  const lessons = await sql`
    SELECT cl.id, cl.what_to_teach, i.description, cl.topic
  ...
  for (const l of lessons) {
    if (l.id === '3aedb974-a9a5-440f-a603-c8b2f8043143' || l.id === 'b8f0e935-5a13-4e57-ad27-83001eda313f') continue;
    
    // Check if what_to_teach lacks "Key concepts for" which means we already updated it
    if (l.what_to_teach && !l.what_to_teach.includes('Key concepts for')) {
      continue;
    }
    
    const c = generateContent(l.description, l.topic);
    ...
    updatedCount++;
    if (updatedCount % 20 === 0) console.log(`Executed ${updatedCount}...`);
  }
