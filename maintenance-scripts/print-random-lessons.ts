...
async function run() {
  ...
  for (const classLevel of ['BASIC 4', 'BASIC 5', 'BASIC 6']) {
    const randomLessons = await sql`
      SELECT cl.id 
      FROM curriculum_lessons cl
      JOIN subjects subj ON cl.subject_id = subj.id
      JOIN class_levels c ON cl.class_level_id = c.id
      WHERE subj.name = 'Mathematics' AND c.name = ${classLevel}
      ORDER BY random() LIMIT 2
    `;
    
    for (const l of randomLessons) {
      await queryLesson(sql, l.id);
    }
  }
