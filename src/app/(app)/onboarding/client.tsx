        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
          ...
          <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              ...
              <button type="button" onClick={addAssignment} disabled={!selectedClass || !selectedSubject} className="w-full sm:w-auto bg-gray-900 dark:bg-gray-700 text-white px-4 py-2 rounded-lg disabled:opacity-50 hover:bg-gray-800 transition-colors">
                Add
              </button>
            </div>
                ...
                {assignments.map(a => {
                  ...
                  return (
                    <li key={`${a.classLevelId}-${a.subjectId}`} className="flex items-center justify-between bg-white dark:bg-gray-900 p-2.5 rounded-lg border shadow-sm">
                      ...
                      <button type="button" onClick={() => removeAssignment(a.classLevelId, a.subjectId)} className="text-red-500 hover:bg-red-50 p-1 rounded transition-colors">
          ...
          <div className="pt-4 flex flex-col sm:flex-row sm:justify-end">
            <button 
              ...
              className="w-full sm:w-auto bg-blue-600 text-white px-6 py-2.5 rounded-lg font-medium hover:bg-blue-700 transition shadow-sm"
            >
              Next Step
            </button>
          </div>
      ...
      {step === 2 && (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
          ...
          <div className="pt-4 flex flex-col-reverse sm:flex-row sm:justify-between gap-3">
            <button type="button" onClick={() => setStep(1)} className="w-full sm:w-auto border px-6 py-2.5 rounded-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition">
              Back
            </button>
            <button type="submit" disabled={isPending || assignments.length === 0 || !globalTermId} className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-2.5 rounded-lg font-medium hover:opacity-90 transition disabled:opacity-50 flex justify-center items-center gap-2 shadow-sm">
              {isPending ? (
                <>
